...
def process_request(request):
    try:
        ...
        replies = []
        for call in calls:
            logger.log_text(f"Will process bq call " + str(call), severity="INFO")
            ...
            data_level_field.enum_value.display_name = table_level
            tag.fields["data_level"] = data_level_field

            data_domain_field = datacatalog_v1.TagField()
            data_domain_field.string_value = table_domain
            tag.fields["data_domain"] = data_domain_field
            ...
            get_tags_page_result = datacatalog_client.list_tags(
            ...
            call_result = {
                ...
                "status": "SUCCESS",
                "data_catalog_method": data_catalog_method,
            }
            replies.append(call_result)

        return_json = jsonify({"replies": replies})
        logger.log_text(f"Function call ending. Replies {replies}", severity="INFO")
        return return_json
